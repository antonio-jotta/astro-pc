...
fractional memory. The passive and full Astro-PC variants keep the configured
fractional-memory setting, because they represent the full Astro-PC package and
its passive null.
...
def variant_specs(base_cfg: Config) -> list[dict[str, Any]]:
    """Compact comparison: PC K=0, passive null, full Astro-PC."""
    ...
    return [
        ...
        {
            "name": "passive_astro_pc",
            "label": "Passive Astro-PC null",
            "overrides": {
...
def summarize_forgetting_per_seed(forgetting_df: pd.DataFrame) -> pd.DataFrame:
    if forgetting_df.empty:
        return pd.DataFrame()
    return (
        forgetting_df.groupby(["seed", "model_name", "model_label"], as_index=False)
        .agg(
...
def aggregate_context_summary(per_seed: pd.DataFrame) -> pd.DataFrame:
    ...
    return agg.sort_values("mean_forgetting_mean")
...
def aggregate_final_context_table(per_seed_table: pd.DataFrame) -> pd.DataFrame:
    ...
    return per_seed_table.groupby(["model_name", "model_label"], as_index=False).agg(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Available names: pc_K0, passive_astro_pc, full_astro_pc."
        ),
    )
    parser.add_argument(
        "--force_restart",
...
def main() -> None:
    ...
    for seed in seeds:
        ...
        specs = variant_specs(base_cfg)

        if args.variants.strip().lower() != "all":
            requested_variants = {
                name.strip() for name in args.variants.split(",") if name.strip()
            }
            specs = [spec for spec in specs if spec["name"] in requested_variants]

            if not specs:
                valid_names = [spec["name"] for spec in variant_specs(base_cfg)]
                raise ValueError(
                    f"No matching variants for --variants={args.variants!r}. "
                    f"Valid options are: {valid_names} or 'all'."
                )
    ...
    write_aggregate_outputs(out_root, all_eval_rows, all_train_rows)
