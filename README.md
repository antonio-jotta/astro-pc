...
compute_paired_stats.py      Paired seed-level statistical summary
```
...
```text
Predictive coding        Plain predictive coding baseline with K=0 and no astrocyte controller
Passive Astro-PC null    Slow-gain null model without residual-sensitive astrocyte-state feedback
Astro-PC                 Complete active Astro-PC configuration
```

The static corrupted-MNIST experiment also includes the additive component-ablation ladder:

```text
Predictive coding        Baseline PC learner with no memory or astrocyte controller
PC + memory              PC with the finite power-law plasticity trace
Astro gain only          PC + memory with residual-sensitive astrocyte-state gain modulation
+ state coupling         Adds local coupling between astrocyte-like controller states
+ leakage                Adds spatial leakage of the effective plasticity gains
Astro-PC                 Adds phagocytosis-like pruning to the complete active configuration
```

The corresponding run-directory names are:
...
The passive Astro-PC null is included as an auxiliary comparison, but it is not part of the additive ladder.
...
To run only a subset of variants, use `--variants`. For example:

```bash
python exp_static_corruptions.py \
  --out_dir results/mnist_static_corrupted_5seed_v1 \
  --seeds 0,1,2,3,4 \
  --variants pc_memory_K2,astro_gain_only \
  --gaussian_std 0.275 \
  --motion_kernel 11 \
  --pixel_size 5 \
  --brightness_delta 0.25 \
...
The FGSM script evaluates the same model variants trained by `exp_static_corruptions.py`, including the component-ablation ladder and the passive Astro-PC null.

Expected outputs include:

```text
results/mnist_static_corrupted_5seed_v1/fgsm_eval/fgsm_mnist_raw_results.csv
results/mnist_static_corrupted_5seed_v1/fgsm_eval/fgsm_mnist_summary_long.csv
results/mnist_static_corrupted_5seed_v1/fgsm_eval/fgsm_mnist_accuracy_table.csv
results/mnist_static_corrupted_5seed_v1/fgsm_eval/fgsm_mnist_accuracy_table.tex
```
...
This experiment uses the same primary comparison and component-ablation ladder described in Section 3.1. The passive Astro-PC null is again included as an auxiliary comparison rather than as part of the additive ladder.

The `--variants` argument can also be used here to split long ablation runs across terminals. For example:
...
python compute_paired_stats.py \
  ...
  --fgsm_csv results/mnist_static_corrupted_5seed_v1/fgsm_eval/fgsm_mnist_raw_results.csv \
