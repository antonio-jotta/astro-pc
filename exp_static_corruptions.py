...
table reporting final clean and corrupted accuracy averaged over seeds.
...
def variant_configs(base_cfg: Config, seed: int) -> list[tuple[str, str, Config]]:
    """
    Return the three variants used in the paper table.

    - PC: plain predictive coding with K=0 and no astrocyte controller.
    - Passive Astro-PC null: same Astro-PC package as the configured model,
      but with passive slow-gain dynamics.
    - Full Astro-PC: the configured active astrocyte-state controller.
    """
    common = dict(
        seed=seed,
        dataset="mnist",
    ...
    return [
        ("pc_K0", "PC", pc),
        ("passive_astro_pc", "Passive Astro-PC null", passive),
        ("full_astro_pc", "Full Astro-PC", full),
    ]
...
def make_summary_tables(raw_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    table_summary["model_order"] = table_summary["model_name"].map(model_order)
...
def main() -> None:
    ...
    for seed in seeds:
        set_seed(seed)
        for model_name, model_label, cfg in variant_configs(base_cfg, seed):
